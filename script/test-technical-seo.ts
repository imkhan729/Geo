...
async function runAllTests() {
  ...
  await runTest("All 21 canonical routes defined with self-referencing canonical URLs", () => {
    const entries = Object.entries(SEO_CONFIG);
    assert.strictEqual(entries.length, 21, `Expected 21 routes, got ${entries.length}`);
  ...
  await runTest("All 21 pages have unique titles (50-60 chars) and descriptions (140-160 chars)", () => {
  ...
  await runTest("sitemap.xml includes all 21 canonical URLs with valid priorities and https protocol", () => {
    ...
    assert.strictEqual(locMatches.length, 21, `Expected 21 sitemap URLs, got ${locMatches.length}`);
    ...
    assert.ok(sitemap.includes("<loc>https://freegeotagger.com/batch-geotag-photos</loc>"));
  });
  ...
  await runTest(".htaccess enforces HTTPS, non-www, index.html redirect, trailing slash strip, and 20 SEO routes", () => {
    ...
    assert.strictEqual(routesCount, 20, `Expected 20 subpage rewrites, got ${routesCount}`);
  });
