...
async function runAllTests() {
  ...
  await runTest("All 17 canonical routes defined with self-referencing canonical URLs", () => {
    const entries = Object.entries(SEO_CONFIG);
    assert.strictEqual(entries.length, 17, `Expected 17 routes, got ${entries.length}`);
  ...
  await runTest("All 17 pages have unique titles (50-60 chars) and descriptions (140-160 chars)", () => {
  ...
  await runTest("sitemap.xml includes all 17 canonical URLs with valid priorities and https protocol", () => {
    ...
    assert.strictEqual(locMatches.length, 17, `Expected 17 sitemap URLs, got ${locMatches.length}`);
  ...
  await runTest(".htaccess enforces HTTPS, non-www, index.html redirect, trailing slash strip, and 16 SEO routes", () => {
    ...
    assert.strictEqual(routesCount, 16, `Expected 16 subpage rewrites, got ${routesCount}`);
  });
